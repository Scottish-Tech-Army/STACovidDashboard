...
import SingleValueBarContainer, { parseNhsCsvData } from "./SingleValueBar";
...
jest.mock("../components/SingleValue/SingleValueBar", () => {
  return {
    __esModule: true,
...
const DATE_TODAY = Date.parse("2020-10-17");
...
test("singleValueBar renders default data when dataset is null", async () => {
  await act(async () => {
    render(
      <SingleValueBarContainer currentTotalsHealthBoardDataset={null} />,
      container
    );
  });

  expect(SingleValueBar).toHaveBeenLastCalledWith(
    {
      dailyCases: undefined,
      dailyDeaths: undefined,
...
test("singleValueBar renders with data", async () => {
  await act(async () => {
    render(
      <SingleValueBarContainer
...
test("parseNhsCsvData", () => {
  const expectedResult = {
    cases: { date: DATE_TODAY, value: 1167 },
    deaths: { date: DATE_TODAY, value: 15 },
