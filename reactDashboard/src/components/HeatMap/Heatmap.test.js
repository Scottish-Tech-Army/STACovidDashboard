...
const dailyHealthBoardWithScotlandCsvData = readCsvData(`Date,HB,HBName,DailyPositive,CumulativePositive,CrudeRatePositive,CumulativePositivePercent,DailyDeaths,CumulativeDeaths,CrudeRateDeaths,CumulativeNegative,CrudeRateNegative
    ...
    `);
const dailyHealthBoardWithoutScotlandCsvData = readCsvData(`Date,HB,HBName,DailyPositive,CumulativePositive,CrudeRatePositive,CumulativePositivePercent,DailyDeaths,CumulativeDeaths,CrudeRateDeaths,CumulativeNegative,CrudeRateNegative
    20200306,S08000031,unknown,0,21,0,0,1,10,0,0,0
    20200306,S08000022,unknown,1,22,0,0,2,20,0,0,0
    20200306,S08000020,unknown,1,23,0,0,3,30,0,0,0
    20200309,S08000031,unknown,0,24,0,0,4,40,0,0,0
...
describe("heatmap renders dynamic fetched data", () => {
  it("council areas; deaths", async () => {
    ...
    // counts = 6, -3, 24, 15
    checkRow(dataRows[0], "Scotland", "", [3, 0, 5, 4]);
    // counts = 2, -1, 8, 5
  ...
  it("health boards; deaths (with Scotland)", async () => {
    ...
    expect(dataRows).toHaveLength(4);
    // counts = 73, 76, 75, 74
    checkRow(dataRows[0], "Scotland", "", [6]);
    // counts = 3, -2, 9, 6
  ...
  it("health boards; cases (with Scotland)", async () => {
    ...
    expect(dataRows).toHaveLength(4);
    // counts = 51, -51, 57, 51
    checkRow(dataRows[0], "Scotland", "", [6, 0, 6]);
    // counts = 1, -1, 26, -8
  ...
  it("health boards; cases (without Scotland)", async () => {
    ...
    expect(dataRows).toHaveLength(4);
    // counts = 2, -2, 227, 292
    checkRow(dataRows[0], "Scotland", "", [2, 0, 7]);
    // counts = 1, -1, 26, -8
    checkRow(dataRows[1], "Grampian", "26", [1, 0, 5, 0]);
    // counts = 0, 0, 0, 0
    checkRow(dataRows[2], "Greater Glasgow & Clyde", "24", [0]);
